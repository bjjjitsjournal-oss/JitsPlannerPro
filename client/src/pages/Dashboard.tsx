...
export default function Dashboard() {
  ...
  return (
    ...
    <div className="p-6 max-w-md mx-auto min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      {/* Recent Quick Wins */}
      {Array.isArray(notesData) && notesData.filter((n: any) => n.title === 'Quick Win 🏆').length > 0 && (
            ...
            {notesData
              ...
              .map((note: any) => (
                <div key={note.id} className="flex items-center gap-3 
                  ...
                  <span className="text-xl">{note.content?.split(' ')[0]}</span>
                  <span className="text-sm text-gray-200 flex-1">
                    {note.content?.split(' ').slice(1).join(' ')}
                  </span>
                  <span className="text-xs text-gray-500">
                    {new Date(note.createdAt || note.created_at).toLocaleDateString('en-AU', { 
