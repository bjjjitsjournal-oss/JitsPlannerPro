...
// Falls back to Capacitor Preferences if supabase.auth.getSession() returns null
// (happens on Android after app updates when Supabase's own storage resets)
async function getAuthInfo(): Promise<{ token: string | null; supabaseId: string | null }> {
  try {
    ...
    console.warn('supabase.auth.getSession() failed, falling back to Preferences:', e);
  }

  // Fallback: read directly from Capacitor Preferences (mobile) or localStorage (web)
  if (Capacitor.isNativePlatform()) {
    try {
      const [tokenResult, idResult] = await Promise.all([
        Preferences.get({ key: 'supabase_access_token' }),
        Preferences.get({ key: 'supabase_user_id' }),
      ]);
      return {
        token: tokenResult.value || null,
        supabaseId: idResult.value || null,
      };
    } catch (e) {
      console.error('Failed to read auth from Preferences:', e);
    }
  } else {
    return {
      token: localStorage.getItem('supabase_access_token'),
      supabaseId: localStorage.getItem('supabase_user_id'),
    };
  }

  return { token: null, supabaseId: null };
}
