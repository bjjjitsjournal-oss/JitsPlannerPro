...
// Hybrid storage: Database for most features, Memory for notes (due to UUID complexity)
class HybridStorage implements IStorage {
  private dbStorage = new DatabaseStorage();
  private memStorage = new MemStoragePrimary();

  // For everything except notes, use database storage
  async getUser(id: number): Promise<User | undefined> {
  ...
  // Classes - use database
  async getClasses(userId?: number): Promise<Class[]> {
    return this.dbStorage.getClasses(userId);
  }

  async getClass(id: number): Promise<Class | undefined> {
    return this.dbStorage.getClass(id);
  }
  ...
  // Videos - use database
  async getVideos(userId?: number): Promise<Video[]> {
    return this.dbStorage.getVideos(userId);
  }

  async getVideo(id: number): Promise<Video | undefined> {
    return this.dbStorage.getVideo(id);
  }

  async getVideosByCategory(category: string, userId?: number): Promise<Video[]> {
    return this.dbStorage.getVideosByCategory(category, userId);
  }

  async searchVideos(query: string, userId?: number): Promise<Video[]> {
    return this.dbStorage.searchVideos(query, userId);
  }

  async createVideo(videoData: InsertVideo): Promise<Video> {
    return this.dbStorage.createVideo(videoData);
  }

  async updateVideo(id: number, videoData: Partial<InsertVideo>): Promise<Video | undefined> {
    return this.dbStorage.updateVideo(id, videoData);
  }

  async deleteVideo(id: number): Promise<boolean> {
    return this.dbStorage.deleteVideo(id);
  }

  // Notes - use memory storage to avoid UUID issues
  async getNotes(userId?: string): Promise<Note[]> {
    // Convert string userId to integer for memory storage compatibility
    const numericUserId = userId ? parseInt(userId) : undefined;
    return this.memStorage.getNotes(numericUserId ? numericUserId.toString() : undefined);
  }
  ...
  async searchNotes(query: string, userId?: string): Promise<Note[]> {
    const numericUserId = userId ? parseInt(userId) : undefined;
    return this.memStorage.searchNotes(query, numericUserId ? numericUserId.toString() : undefined);
  }

  async createNote(noteData: InsertNote): Promise<Note> {
    // Convert string userId back to integer for memory storage
    const convertedData = {
      ...noteData,
      userId: typeof noteData.userId === 'string' ? parseInt(noteData.userId) : noteData.userId
    };
    return this.memStorage.createNote(convertedData as InsertNote);
  }
  ...
  async updateNote(id: number, noteData: Partial<InsertNote>): Promise<Note | undefined> {
    ...
    return this.memStorage.updateNote(id, convertedData as Partial<InsertNote>);
  }
  ...
  async shareNote(noteId: number, targetUserId: number): Promise<boolean> {
    return this.memStorage.shareNote(noteId, targetUserId);
  }

  // Drawings - use database
  async getDrawing(id: number): Promise<Drawing | undefined> {
    return this.dbStorage.getDrawing(id);
  }

  async createDrawing(drawingData: InsertDrawing): Promise<Drawing> {
    return this.dbStorage.createDrawing(drawingData);
  }

  async updateDrawing(id: number, drawingData: Partial<InsertDrawing>): Promise<Drawing | undefined> {
    return this.dbStorage.updateDrawing(id, drawingData);
  }

  async deleteDrawing(id: number): Promise<boolean> {
    return this.dbStorage.deleteDrawing(id);
  }

  async getDrawings(userId?: number): Promise<Drawing[]> {
    return this.dbStorage.getDrawings(userId);
  }

  // Belts - use database
  async getBelts(userId?: number): Promise<Belt[]> {
    return this.dbStorage.getBelts(userId);
  }

  async getBelt(id: number): Promise<Belt | undefined> {
    return this.dbStorage.getBelt(id);
  }

  async getCurrentBelt(userId?: number): Promise<Belt | undefined> {
    return this.dbStorage.getCurrentBelt(userId);
  }

  async createBelt(beltData: InsertBelt): Promise<Belt> {
    return this.dbStorage.createBelt(beltData);
  }

  async updateBelt(id: number, beltData: Partial<InsertBelt>): Promise<Belt | undefined> {
    return this.dbStorage.updateBelt(id, beltData);
  }

  async deleteBelt(id: number): Promise<boolean> {
    return this.dbStorage.deleteBelt(id);
  }

  // Statistics - use database
  async getClassStats(userId?: number): Promise<{
    totalClasses: number;
    lastPromotionDate?: string;
    currentBelt?: string;
    currentStripes?: number;
  }> {
    return this.dbStorage.getClassStats(userId);
  }

  // Weekly Commitments - use database
  async getWeeklyCommitments(userId?: number): Promise<WeeklyCommitment[]> {
    return this.dbStorage.getWeeklyCommitments(userId);
  }

  async getCurrentWeekCommitment(userId?: number): Promise<WeeklyCommitment | undefined> {
    return this.dbStorage.getCurrentWeekCommitment(userId);
  }

  async createWeeklyCommitment(commitmentData: InsertWeeklyCommitment): Promise<WeeklyCommitment> {
    return this.dbStorage.createWeeklyCommitment(commitmentData);
  }

  async updateWeeklyCommitment(id: number, commitmentData: Partial<InsertWeeklyCommitment>): Promise<WeeklyCommitment | undefined> {
    return this.dbStorage.updateWeeklyCommitment(id, commitmentData);
  }

  async deleteWeeklyCommitment(id: number): Promise<boolean> {
    return this.dbStorage.deleteWeeklyCommitment(id);
  }

  // Training Videos - use database
  async getTrainingVideos(): Promise<TrainingVideo[]> {
    return this.dbStorage.getTrainingVideos();
  }

  async getTrainingVideo(id: number): Promise<TrainingVideo | undefined> {
    return this.dbStorage.getTrainingVideo(id);
  }

  async createTrainingVideo(videoData: InsertTrainingVideo): Promise<TrainingVideo> {
    return this.dbStorage.createTrainingVideo(videoData);
  }

  async updateTrainingVideo(id: number, videoData: Partial<InsertTrainingVideo>): Promise<TrainingVideo | undefined> {
    return this.dbStorage.updateTrainingVideo(id, videoData);
  }

  async deleteTrainingVideo(id: number): Promise<boolean> {
    return this.dbStorage.deleteTrainingVideo(id);
  }

  // Enhanced Note Sharing - use memory storage for notes
  async toggleNoteSharing(noteId: number, userId: number): Promise<Note | undefined> {
    return this.memStorage.toggleNoteSharing(noteId, userId);
  }

  async getSharedNotes(): Promise<Note[]> {
    return this.memStorage.getSharedNotes();
  }

  // Password Reset Tokens - use database
  async createPasswordResetToken(userId: number, token: string, expiresAt: Date): Promise<PasswordResetToken> {
    return this.dbStorage.createPasswordResetToken(userId, token, expiresAt);
  }

  async getPasswordResetToken(token: string): Promise<PasswordResetToken | undefined> {
    return this.dbStorage.getPasswordResetToken(token);
  }

  async markPasswordResetTokenAsUsed(tokenId: number): Promise<void> {
    return this.dbStorage.markPasswordResetTokenAsUsed(tokenId);
  }
  ...
  // Note Likes - use memory storage for notes-related features
  async likeNote(noteId: number, userId: number): Promise<boolean> {
    return this.memStorage.likeNote(noteId, userId);
  }

  async unlikeNote(noteId: number, userId: number): Promise<boolean> {
    return this.memStorage.unlikeNote(noteId, userId);
  }

  async getNoteLikes(noteId: number): Promise<NoteLike[]> {
    return this.memStorage.getNoteLikes(noteId);
  }

  async getUserNoteLikes(userId: number): Promise<NoteLike[]> {
    return this.memStorage.getUserNoteLikes(userId);
  }

  async isNoteLikedByUser(noteId: number, userId: number): Promise<boolean> {
    return this.memStorage.isNoteLikedByUser(noteId, userId);
  }

  async getNoteWithLikes(noteId: number, userId?: number): Promise<Note & { likeCount: number; isLikedByUser: boolean } | undefined> {
    return this.memStorage.getNoteWithLikes(noteId, userId);
  }
}

// Use hybrid storage: Database for persistent data, Memory for notes
